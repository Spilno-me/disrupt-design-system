...
import { AdvancedConditionalLogic } from './AdvancedConditionalLogic';
...
export function FieldPropertiesPanel({ className }: FieldPropertiesPanelProps) {
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      {/* Advanced Conditional Logic (AND/OR Groups) */}
      {selectedField.type !== 'void' && (
        <>
          <Separator />
      ...
      return (
        <>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="calc-formula" className="text-sm">
                Formula
              </Label>
              <Input
                id="calc-formula"
                value={(componentProps.formula as string) || ''}
                onChange={(e) => updateComponentProps({ formula: e.target.value })}
                placeholder="e.g., likelihood * severity"
                className="font-mono text-sm"
              />
              <p className="text-xs text-text-secondary">
                Use field names from this form in your expression
              </p>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="calc-source-fields" className="text-sm">
                Source Fields
              </Label>
              <Input
                id="calc-source-fields"
                value={((componentProps.sourceFields as string[]) || []).join(', ')}
                onChange={(e) => updateComponentProps({
                  sourceFields: e.target.value.split(',').map(s => s.trim()).filter(Boolean)
                })}
                placeholder="e.g., likelihood, severity"
              />
              <p className="text-xs text-text-secondary">
                Comma-separated list of field names used in the formula
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="calc-result-type" className="text-sm">
                  Result Type
                </Label>
                <select
                  id="calc-result-type"
                  value={(componentProps.resultType as string) || 'number'}
                  onChange={(e) => updateComponentProps({ resultType: e.target.value })}
                  className="w-full h-9 px-3 py-2 text-sm rounded-md border border-border-default bg-surface"
                >
              ...
              <div className="space-y-1.5">
                <Label htmlFor="calc-decimals" className="text-sm">
                  Decimal Places
                </Label>
                <Input
                  id="calc-decimals"
                  ...
                  min={0}
                  max={10}
                  value={(componentProps.decimalPlaces as number) || 2}
                  onChange={(e) =>
                    updateComponentProps({ decimalPlaces: parseInt(e.target.value) || 2 })
                  }
                />
              </div>
            </div>
