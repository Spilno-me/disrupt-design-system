 ...
 * Supports nested paths like ['properties', 'section', 'properties', 'field']
 * @param schema - The root schema
 * @param path - Path array like ['properties', 'firstName'] or nested paths
 */
export function deleteSchemaProperty(schema: ISchema, path: string[]): ISchema {
  if (path.length < 2 || path[0] !== 'properties') return schema;

  // Simple case: top-level field ['properties', 'fieldName']
  if (path.length === 2) {
    const fieldName = path[1];
  ...
  // Nested case: traverse and delete
  // Path like ['properties', 'section', 'properties', 'field']
  // or ['properties', 'array', 'items', 'properties', 'field']
  const newSchema = JSON.parse(JSON.stringify(schema)) as ISchema;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let current: any = newSchema;
  ...
  for (let i = 0; i < path.length - 2; i++) {
    ...
    if (!current) return schema; // Path doesn't exist
  }

  // The last two elements are 'properties' and the field name
  const propertiesKey = path[path.length - 2];
  const fieldName = path[path.length - 1];

  if (propertiesKey !== 'properties' || !current[propertiesKey]) {
    return schema;
  }

  // Delete the field
  const { [fieldName]: _, ...remainingProperties } = current[propertiesKey];
  ...
  for (const [key, prop] of Object.entries(remainingProperties)) {
    reindexedProperties[key] = {
      ...(prop as SchemaProperty),
      'x-index': index++,
    };
  }

  current[propertiesKey] = reindexedProperties;
