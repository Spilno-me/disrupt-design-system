...
export type FieldCategory = 'form' | 'data' | 'dictionary' | 'layout';
...
> = {
  // ABYSS palette - Purple (Layout & Structure)
  layout: {
    iconBg: 'bg-[var(--brand-abyss-100)] dark:bg-[var(--brand-abyss-900)]',
    text: 'text-[var(--brand-abyss-600)] dark:text-[var(--brand-abyss-300)]',
    headerBg: 'bg-[var(--brand-abyss-50)]/50 dark:bg-[var(--brand-abyss-900)]/30',
    headerBorder: 'border-l-[var(--brand-abyss-500)]',
    badge: 'bg-abyss-100 text-abyss-700 border-abyss-200',
  },
...
export interface ConditionalReaction {
  ...
  /** Advanced conditional logic with AND/OR groups */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _advancedConditional?: any;
}
...
export type VisibilityCondition = 'equals' | 'notEquals' | 'hasValue' | 'isEmpty' | 'greaterThan' | 'lessThan' | 'contains';
...
/** Advanced condition group (AND/OR) */
...
/** Advanced conditional visibility with multiple condition groups */
export interface AdvancedConditionalVisibility {
  enabled: boolean;
  groups: ConditionGroup[];
  groupOperator: LogicOperator; // How groups are combined
  action: VisibilityAction;
}
