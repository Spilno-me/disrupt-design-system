...
<SectionHeader
  icon={<Wrench size={20} />}
  title="13 Tools Available"
  description="Component lookup, token validation, color intelligence, and WCAG contrast checking."
/>

{/* Core Tools */}
<h3 style={{ fontSize: TYPOGRAPHY.fontSize.sm[0], fontWeight: TYPOGRAPHY.fontWeight.semibold, color: ABYSS[500], marginBottom: SPACING.px.base }}>
  Core Tools
</h3>
...
<div style={{
  ...
  {[
    ...
    { name: 'get_design_philosophy', icon: <BookOpen size={20} />, desc: 'Wu Wei + MAYA + QoE principles', example: 'get_design_philosophy({})' },
  ].map((tool) => (
    <div
      key={tool.name}
      style={{
...
<div style={{
  ...
  {[
    { name: 'check_contrast', icon: <Eye size={20} />, desc: 'WCAG contrast ratio between two colors', example: 'check_contrast({background: "ABYSS[900]", foreground: "PRIMITIVES.white"})' },
    { name: 'get_accessible_colors', icon: <ShieldCheck size={20} />, desc: 'Find colors meeting WCAG requirements', example: 'get_accessible_colors({background: "ABYSS[900]", minLevel: "AAA"})' },
    { name: 'list_color_tokens', icon: <Palette size={20} />, desc: 'List available color tokens', example: 'list_color_tokens({filter: "ABYSS"})' },
  ].map((tool) => (
    <div
      key={tool.name}
      style={{
...
<div style={{
  ...
  {[
    { name: 'get_color_recommendation', icon: <Target size={20} />, desc: 'Context-aware color recommendations for 13 UI contexts (card, modal, button, etc.)', example: 'get_color_recommendation({context: "card", theme: "light"})' },
    { name: 'get_color_harmony', icon: <Layers size={20} />, desc: 'Color harmony rules (companions, accents, colors to avoid)', example: 'get_color_harmony({color: "CORAL"})' },
    { name: 'validate_color_choice', icon: <ShieldCheck size={20} />, desc: 'Validate foreground/background combinations with WCAG + forbidden patterns', example: 'validate_color_choice({background: "ABYSS[900]", foreground: "CORAL[200]"})' },
    { name: 'get_glass_rules', icon: <Layers size={20} />, desc: 'Glass/frosted UI styling rules by depth level (1-3)', example: 'get_glass_rules({depth: 1})' },
  ].map((tool) => (
    <div
      key={tool.name}
      style={{
...
<div style={{
  ...
  {[
    { phase: 'Phase 1', status: 'COMPLETE', color: DEEP_CURRENT[500], bg: DEEP_CURRENT[50], title: 'Foundation + Color Intelligence', desc: '13 tools: component lookup, token validation, WCAG contrast, color intelligence', tools: 'get_component, search_components, get_design_tokens, get_color_guidance, check_token_usage, get_design_philosophy, check_contrast, get_accessible_colors, list_color_tokens, get_color_recommendation, get_color_harmony, validate_color_choice, get_glass_rules' },
    { phase: 'Phase 2', status: 'NEXT', color: DUSK_REEF[500], bg: DUSK_REEF[50], title: 'Patterns & Workflows', desc: 'Storybook patterns, testing guides, component workflows', tools: 'get_storybook_patterns, get_component_workflow, get_testing_patterns' },
...
├── src/
│   ├── index.ts              # Main server with core tools
│   └── tools/
│       ├── colorRecommendation.ts  # Context-aware color recommendations
│       ├── colorHarmony.ts         # Color harmony rules
│       ├── colorValidation.ts      # Validate color combinations
│       └── glassRules.ts           # Glass/frosted UI styling
...
├── .claude/contrast-matrix.json    # WCAG contrast combinations
└── src/data/color-intelligence.json # Full color intelligence system`}</pre>
  </div>
</div>
