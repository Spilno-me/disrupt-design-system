...
Button.displayName = 'Button'`}
  ...
  <div style={{
    ...
    <strong>Key:</strong> Extend <code style={{ background: PRIMITIVES.white, padding: '1px 4px', borderRadius: RADIUS.xs }}>React.HTMLAttributes</code> and spread <code style={{ background: PRIMITIVES.white, padding: '1px 4px', borderRadius: RADIUS.xs }}>...props</code> to accept data-testid
  </div>
</div>
...
function LeadCard({ lead, testId }: LeadCardProps) {
  ...
  <div style={{
    ...
    <strong>Key:</strong> Auto-generate from entity ID with optional <code style={{ background: PRIMITIVES.white, padding: '1px 4px', borderRadius: RADIUS.xs }}>testId</code> override prop
  </div>
</div>
