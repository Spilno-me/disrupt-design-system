...
*Permission to write bad code first. QoE: Allow ugliness.*
...
*Generate a new prompt following DDS prompt library patterns and conventions.*
...
*Review code against Uncle Bob clean code principles for production readiness.*
...
*Analyze a God File (>500 lines) and create extraction plan.*
...
*Create mock API layer with seed data for Storybook and testing.*
...
*Add data-testid attributes to components following DDS three-layer testing strategy.*
...
*Create unit tests for component utilities, transformations, and pure functions.*
...
*Create Storybook play() function tests for user flows and interactions.*
...
*Audit test coverage for a component: testId attributes, unit tests, and interaction tests.*
...
*Apply correct depth layering with elevation, shadows, and backgrounds.*
...
*Apply correct spacing using DDS spacing tokens.*
...
*Apply correct typography using DDS font scale and weights.*
...
*Apply correct color token priority: semantic > contextual > primitive.*
...
*Ensure component works correctly in dark mode.*
...
*Apply Fitts, Hick, Miller, and Gestalt principles to UI.*
...
*Replace inaccessible patterns with semantic HTML and Radix.*
...
*Apply iOS Safari-specific fixes and safe-area utilities.*
...
*Apply appropriate responsive strategy based on UI complexity.*
...
*Convert emoji usage to Lucide React icons.*

**Variables:** `{COMPONENT}`

```
Replace emojis with Lucide icons in {COMPONENT}.
...
*Create a standalone delivery package for a component.*
...
*Create adapter functions to map application data to DDS component interfaces.*
...
*Create a wrapper component to encapsulate DDS integration with app-specific behavior.*
...
*Follow semantic versioning workflow to publish DDS package updates.*
...
*Set up local DDS development workflow with yalc for hot-reload or npm pack for verification.*
...
*Clean up local DDS links and restore npm registry version before committing.*
...
*Follow all-or-nothing page migration strategy to DDS components.*
...
*Verify color combinations meet WCAG accessibility standards using MCP.*
...
*Query component variants, status, and props using MCP.*
...
*Get allowed colors for specific contexts using MCP.*
...
*Check if a token or color value is allowed in DDS.*
...
*Retrieve Wu Wei design principles and engineering guidelines.*
...
*Comprehensive contrast check for a specific background category.*
...
*Audit MCP server data files for drift and sync with actual codebase.*
